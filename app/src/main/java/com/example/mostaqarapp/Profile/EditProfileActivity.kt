...
class EditProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val editEmail = findViewById<EditText>(R.id.etEditEmail)
        val editBio = findViewById<EditText>(R.id.etEditBio)
        val editLocation = findViewById<EditText>(R.id.etEditLocation)
        val editPhone= findViewById<EditText>(R.id.etEditPhone)
...
//        Picasso.get().load(user.image).into(editImage)
        ...
        if (!user.image.isNullOrEmpty()) {
            Picasso.get().load(user.image).into(editImage) // ✅ Load image only if it's valid
        } else {
            editImage.setImageResource(R.drawable.click_account) // ✅ Set default image
        }
